...
function generateResponse(input: string, ctx: MarketContext): string {
  ...
  if (q.match(/price|market|benchmark|global|trend|forecast|driver|why|surge|high|low|arabica|commodity/)) {
    return `Global benchmark (FRED Other Mild Arabica): $${priceUsd}/kg.\nRwanda export price (8% specialty premium): ~$${exportUsd.toFixed(2)}/kg.\n30-day forecast: $${fcastUsd}/kg (${dir === 'up' ? '+' : ''}${change}%).\n\nKey price drivers right now:\n1. Global arabica supply tightness — Brazil and Colombia production shortfalls since 2023\n2. Specialty premium compression risk if recession hits discretionary spending\n3. USD strength — RWF/USD rate directly affects local payouts\n4. Rwanda's Q2 main harvest (April–June) typically softens local prices briefly\n\nData source: FRED monthly (Other Mild Arabica), interpolated to daily via ${ctx.forecastSource === 'lstm' ? 'LSTM inference' : 'linear trend regression'}.`;
  }

  // --- Reliability / model ---
  if (q.match(/reliab|accura|confiden|trust|model|lstm|how good|mape|error|certain/)) {
    const isLSTM = ctx.forecastSource === 'lstm';
    return `Model: ${isLSTM ? 'Two-layer LSTM (64 units × 2 → Dense 32 → Dense 1), trained on FRED Other Mild Arabica data.' : 'Linear trend regression on FRED monthly data, interpolated to daily.'}\n\n${isLSTM ? `Training MAPE: 1.08% — strong directional accuracy over 30-day horizons. Confidence decays from ~93% (Day 1) to ~60% (Day 30) using sqrt-growth decay.` : `Current prices ($${priceUsd}/kg) exceed the LSTM training ceiling ($5.70/kg), so the model falls back to trend regression. Directional signals are valid; exact price targets carry higher uncertainty.`}\n\nAverage forecast confidence: ${conf}%.\n\nUse as: directional guidance + timing signal. Do not anchor exact loan amounts to forecast prices — build in a 5–8% buffer.`;
  }
  ...
  if (q.match(/good time|right time|opportun|entry price|now|current|start/)) {
    ...
    return `${isHigh ? '⚠️ Prices are historically elevated.' : '✅ Prices are within normal range.'} Current: $${priceUsd}/kg vs LSTM training ceiling of $5.70/kg — a ${((ctx.currentPriceUsd / 5.70 - 1) * 100).toFixed(0)}% premium above historical norms.\n\n${isHigh ? 'High entry price increases the risk of margin compression if global arabica supply recovers. Favour shorter loan terms (6–12 months) to reduce exposure.' : 'Current prices support solid farmer margins above the $1.75/kg production cost.'}\n\nProfit per kg at today's export price: $${(exportUsd - PROD_COST_USD).toFixed(2)} (${((exportUsd - PROD_COST_USD) / exportUsd * 100).toFixed(0)}% gross margin).`;
  }
